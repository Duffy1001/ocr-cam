 ...
 */
export function readImageData(source, width, height) {
    if (source instanceof ImageData)
        return source;
    const canvas = new OffscreenCanvas(width, height);
    const ctx = canvas.getContext("2d");
    if (!ctx)
        throw new Error("Cannot create OffscreenCanvas 2D context");
 ...
 */
export function preprocessImage(imageData, targetWidth, targetHeight) {
 ...
 */
export function preprocessImagePadded(imageData, resizeW, targetHeight, padW) {
    const srcW = imageData.width;
    const srcH = imageData.height;
    const src = imageData.data;
 ...
 */
export function preprocessImageDetector(imageData, maxW, maxH, multiple) {
    const srcW = imageData.width;
    const srcH = imageData.height;
 ...
 */
export function cropImageData(source, x, y, w, h) {
...
export function rotateImageData90Clockwise(imageData) {
    const srcW = imageData.width;
    const srcH = imageData.height;
    const src = imageData.data;
    const dstW = srcH;
...
export function rotateImageData270Clockwise(imageData) {
    const srcW = imageData.width;
    const srcH = imageData.height;
    const src = imageData.data;
    const dstW = srcH;
