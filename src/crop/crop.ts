...
export function resolveCrop(
  config: CropConfiguration | null,
  sourceWidth: number,
  sourceHeight: number
): ResolvedCropRegion | null {
...
function resolveNormalizedCrop(
  config: { unit: "normalized"; x: number; y: number; width: number; height: number },
  sourceWidth: number,
  sourceHeight: number
): ResolvedCropRegion {
...
function resolveSourcePxCrop(
  ...
  if (!isFinite(config.width) || config.width <= 0) {
    throw new OcrError(
      "INVALID_CROP",
      `Invalid crop width: ${config.width}`
    );
  }
  if (!isFinite(config.height) || config.height <= 0) {
    throw new OcrError(
      "INVALID_CROP",
      `Invalid crop height: ${config.height}`
    );
  }
...
function validateNormalizedValue(value: number, name: string): void {
  if (!isFinite(value)) {
  ...
  if (value < 0 || value > 1) {
    throw new OcrError(
      "INVALID_CROP",
      `Normalized ${name} must be between 0 and 1, got ${value}`
    );
  }
}
...
export function cropRegionToPixelRect(crop: ResolvedCropRegion): PixelRect {
  return {
    x: crop.x,
    y: crop.y,
    width: crop.width,
