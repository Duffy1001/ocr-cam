...
export function extractBoxesFromProbabilityMap(
  prob: Float32Array,
  mapW: number,
  mapH: number,
  srcW: number,
  ...
  for (let y = 0; y < mapH; y++) {
    for (let x = 0; x < mapW; x++) {
      ...
      while (queue.length > 0) {
        ...
        if (cx > 0 && !visited[cur - 1]) { visited[cur - 1] = 1; queue.push(cur - 1); }
        if (cx < mapW - 1 && !visited[cur + 1]) { visited[cur + 1] = 1; queue.push(cur + 1); }
        if (cy > 0 && !visited[cur - mapW]) { visited[cur - mapW] = 1; queue.push(cur - mapW); }
        if (cy < mapH - 1 && !visited[cur + mapW]) { visited[cur + mapW] = 1; queue.push(cur + mapW); }
      }
...
export function boxesToDetections(boxes: DetectionBox[]): OcrDetection[] {
  return boxes.map((b) => ({
    text: "",
