...
import type { InferenceSession } from "../types/ort.js";
import { OcrError } from "../types/errors.js";
import { loadSession, releaseSession, loadOrt } from "./modelLoader.js";
import {
  preprocessImageDetector,
  preprocessImagePadded,
  readImageData,
  cropImageData,
  rotateImageData90Clockwise,
  rotateImageData270Clockwise,
} from "./preprocess.js";
import {
...
export function createOnnxOcrEngine(config: OnnxEngineConfig): OcrEngine {
  ...
  return {
    async load(signal?: AbortSignal, onProgress?: (progress: import("../types/public.js").LoadProgress) => void): Promise<void> {
      if (loaded) return;
    ...
    async recognize(
      ...
      try {
        ...
        const detTensor = new ort.Tensor(
          "float32",
          detPreprocessed.tensorData,
          [1, 3, detPreprocessed.height, detPreprocessed.width]
        );

        const detInputName =
          config.detectorInputName ?? detSession.inputNames[0] ?? "input";
        const detFeeds: Record<string, import("../types/ort.js").Tensor> = {};
        detFeeds[detInputName] = detTensor;

        const detOutput = await detSession.run(detFeeds);

        const detOutputName =
          config.detectorOutputName ?? detSession.outputNames[0] ?? "output";
        ...
        let boxes = extractBoxesFromProbabilityMap(
          probMap,
          mapW,
          mapH,
          frame.width,
        ...
        async function recognizeFromCrop(cropped: ImageData): Promise<{ text: string; score: number }> {
          ...
          const recTensor = new ort.Tensor(
            "float32",
            recPreprocessed.tensorData,
            [1, 3, recH, paddedW]
          );

          const recInputName =
            config.recognizerInputName ?? rec.inputNames[0] ?? "input";
          const recFeeds: Record<string, import("../types/ort.js").Tensor> = {};
          recFeeds[recInputName] = recTensor;

          const recOutput = await rec.run(recFeeds);
          const recOutputName =
            config.recognizerOutputName ?? rec.outputNames[0] ?? "output";
          const logitsTensor = recOutput[recOutputName];
          if (!logitsTensor) return { text: "", score: -Infinity };
        ...
        for (const box of boxes) {
          ...
          if (isTall) {
            ...
            let best = c0;
            if (c90.score > best.score) best = c90;
            if (c270.score > best.score) best = c270;
